...
<template>
  <div v-if="postMetaData" class="post">
    ...
    <div class="post-content">
      <!-- 左侧边栏数据 -->
      <FileListSidebar :files="files" />
      <!-- <FileListSidebar 
        :files="files"
        :debug-info="{ 
          currentDir, 
          currentPath: page.relativePath 
        }"/> -->
...
const currentDir = page.value.relativePath
  ...
  .at(-2)
  // .pop()
console.log('文件夹1',currentDir)
// 响应式的文件列表计算
// 处理文档列表
const files = theme.value.RepositoryData
  .filter(item => {
    const pathSegments = item.regularPath
      .replace('/pages/repository/', '')
      .split('/')
    // console.log('pathSegments',pathSegments)
    return pathSegments.at(-2) === currentDir && pathSegments.length > 1
   
  })
  .map(item => ({
    title: item.title || extractTitleFromPath(item.regularPath),
    link: withBase(item.regularPath.replace('.html', '')),
    meta: {
  ...
  .sort((a, b) => b.meta.modified.localeCompare(a.meta.modified))
console.log('文件夹',currentDir)
...
const postMetaData = computed(() => {
  ...
  return theme.value.RepositoryData.find((item) => item.id === postId);
});
...
.post {
  ...
  .post-content {
    ...
    display: flex;
    flex-direction: row;
    animation: fade-up 0.3s 0.3s backwards;
    gap: 1.5rem; // 添加元素间距
    position: relative;
      // 左侧边栏样式
    .file-list-sidebar {
    ...
    .post-article {
      width: calc(100% - 200px - 250px); // 扣除两侧边栏宽度
      margin: 0 1rem;
